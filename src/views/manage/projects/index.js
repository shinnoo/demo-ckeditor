...
const Project = () => {
  ...
  const getAll = (page = 1, size = 10) => {
    ...
    axios
      .get(
        `${host}/api/v1/projects?page=${page - 1}&size=${size}&sort=id,desc`
      )
      .then((res) => {
  ...
  const getAllDepartment = (page = 1, size = 30) => {
    setIsLoading(true);
    axios
      .get(`${host}/api/v1/departments?page=${page - 1}&size=${size}&sort=id,desc`)
      .then((res) => {
  ...
  const getAllProjectStatus = (page = 1, size = 30) => {
    setIsLoading(true);
    axios
      .get(`${host}/api/v1/project-status/getAllActive?page=${page - 1}&size=${size}&sort=id,desc`)
      .then((res) => {
        setProjectStatus(res.data.items || []);
      })
      .catch((error) => {});
  };

  const getAllProjectType = (page = 1, size = 30) => {
    setIsLoading(true);
    axios
      .get(`${host}/api/v1/project-types/getAllActive?page=${page - 1}&size=${size}&sort=id,desc`)
      .then((res) => {
        setProjectType(res.data.items || []);
      })
      .catch((error) => {});
  };

  const getAllEmployee = (page = 1, size = 30) => {
    setIsLoading(true);
    axios
      .get(
  ...
  const callDelete = () => {
    axios
      .delete(`${host}/api/v1/projects/delete/${selectedId}`)
      .then((res) => {
  ...
  const onFinish = (values) => {
    ...
    values.employee = values.employee != null ? values.employee.join(", ") : "";
    callPut(values);
  };
    ...
    <div>
      ...
      >
        ...
        >
          ...
          <Form.Item name="projectType" label="Loại dự án">
            <Select placeholder="Chọn loại dự án" optionLabelProp="label"
              showArrow="true">
          ...
          <Form.Item name="department" label="Trung tâm">
            <Select placeholder="Chọn trung tâm" optionLabelProp="label"
              showArrow="true">
          ...
          <Form.Item name="projectStatus" label="Trạng thái dự án">
            <Select placeholder="Chọn trạng thái" optionLabelProp="label"
              showArrow="true">
