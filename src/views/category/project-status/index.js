...
import { notification, Button, Table, Tag, Space, Row, Col, Modal, Form, Input, InputNumber,Switch,Typography } from 'antd';
...
const ProjectStatus = () => {
  ...
  const getAll = (page = 1, size = 10) => {
    setIsLoading(true);
    axios.get(`${host}/api/v1/project-status?page=${page - 1}&size=${size}&sort=id,desc`)
      .then(res => {
  ...
  const callDelete = () => {
    axios.delete(`${host}/api/v1/project-status/delete/${selectedId}`)
      .then(res => {
  ...
  const callPut = (data) => {
    ...
    axios.put(`${host}/api/v1/project-status`,data)
      .then(res => {
  ...
  return (
    <div>
      ...
      <Table rowKey="id" columns={columns} dataSource={projectStatus} pagination={paginationConfig} />
