...
const CustomerCategory = () => {
  ...
  const getAll = (page = 1, size = 10) => {
    setIsLoading(true);
    axios.get(`${host}/api/v1/customer-categories?page=${page - 1}&size=${size}&sort=id,desc`)
      .then(res => {
  ...
  const callDelete = () => {
    axios.delete(`${host}/api/v1/customer-categories/delete/${selectedId}`)
      .then(res => {
  ...
  const callPut = (data) => {
    ...
    axios.put(`${host}/api/v1/customer-categories`,data)
      .then(res => {
  ...
  return (
    <div>
      ...
      <Table rowKey="id" columns={columns} dataSource={customerCategories} pagination={paginationConfig} />
