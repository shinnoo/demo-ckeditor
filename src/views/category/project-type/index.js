...
const ProjectType = () => {
  ...
  const getAll = (page = 1, size = 10) => {
    setIsLoading(true);
    axios.get(`${host}/api/v1/project-types?page=${page - 1}&size=${size}&sort=id,desc`)
      .then(res => {
        setProjectTypes(res.data.items || []);
        setIsLoading(false);
        setPaginationConfig({
  ...
  const callDelete = () => {
    axios.delete(`${host}/api/v1/project-types/delete/${selectedId}`)
      .then(res => {
  ...
  const callPut = (data) => {
    ...
    axios.put(`${host}/api/v1/project-types`,data)
      .then(res => {
